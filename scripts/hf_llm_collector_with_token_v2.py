#!/usr/bin/env python3
"""
HF Compliance-Oriented LLM Collector (token-aware, dash-safe)
-------------------------------------------------------------

Adds:
- Visible token verification (masked) and whoami call to confirm auth.
- Hyphen/underscore/slug-aware keyword matching.
- Search query expansion to include dash/underscore/slug variants.

Usage
-----
pip install --upgrade huggingface_hub pandas pyyaml rapidfuzz tqdm

# Preferred (safer): environment variable
export HUGGING_FACE_HUB_TOKEN=hf_********************************
python hf_llm_collector_with_token_v2.py --readme-scan --embed-readme-snippet --sleep 0.25

# Or via CLI flag (less safe due to shell history)
python hf_llm_collector_with_token_v2.py --token hf_xxx --readme-scan
"""
...
def mask_token(t: Optional[str]) -> str:
    if not t:
        return "<none>"
    t = str(t)
    return t[:6] + "..." + t[-4:] if len(t) >= 12 else "<set>"
...
def normalize_blob_text(parts: List[str]) -> Tuple[str, str, str]:
    """
    Returns three variants of the searchable text:
    - blob: lowercased, squashed whitespace
    - blob_dash_spaced: hyphens/underscores/slashes become spaces
    - blob_slug: all spaces/hyphens/underscores/slashes removed
    """
...
def keyword_variants(k: str) -> List[str]:
    k = k.lower()
    variants = {k}
    if " " in k:
        ...
        variants.add(k.replace(" ", ""))
        variants.add(re.sub(r"[ \-_]+", " ", k))  # normalized spaces
    return list(variants)


def fuzzy_contains_any(needles: List[str], haystacks: List[str], threshold: int = 90) -> bool:
    for n in needles:
        for h in haystacks:
...
def match_keywords(blob: str, blob_dash_spaced: str, blob_slug: str) -> Tuple[List[str], List[str], List[str]]:
    pos, frw, task = [], [], []
    haystacks = [blob, blob_dash_spaced, blob_slug]

    for k in CORE_TERMS:
        if fuzzy_contains_any(keyword_variants(k), haystacks): pos.append(k)
    for k in FRAMEWORKS:
        if fuzzy_contains_any(keyword_variants(k), haystacks): frw.append(k)
    for k in TASK_TERMS + SAFETY_POLICY_TERMS:
        if fuzzy_contains_any(keyword_variants(k), haystacks): task.append(k)
    return pos, frw, task


def has_negative(blob: str, blob_dash_spaced: str) -> bool:
...
def parse_args() -> Args:
    ap = argparse.ArgumentParser("HF Compliance LLM collector (token-aware, dash-safe)")
    ...
    ap.add_argument("--no-query-expansion", action="store_true",
                    help="Disable dash/underscore/slug expansion of search queries.")

    a = ap.parse_args()

    base_queries = SEARCH_QUERIES_DEFAULT + list(a.extra_query)
    if a.no_query_expansion:
        queries = list(dict.fromkeys(base_queries))
    else:
        exp_set = set()
        for q in base_queries:
            exp_set.update(expand_query_variants(q))
        queries = list(dict.fromkeys(exp_set))  # keep order-ish while deduping
    ...
    return Args(
        ...
        queries=queries,
        no_query_expansion=a.no_query_expansion,
    )
...
def harvest(args: Args):
    ...
    # Visible, safe auth info
    print(f"[auth] token: {mask_token(token)}")
    try:
        who = api.whoami(token=token) if token else None
        if who:
            user_display = who.get("name") or who.get("user", "<unknown>")
            print(f"[auth] authenticated as: {user_display}")
        else:
            print("[auth] running anonymously (no token provided)")
    except Exception as e:
        print(f"[auth] token provided but whoami failed: {e}")
    ...
    for q in args.queries:
        ...
        for model in tqdm(iter_models_with_retries(
            ...
            # Build searchable blobs (dash-aware)
            blob, blob_dash_spaced, blob_slug = normalize_blob_text([
            ...
            pos, frw, task = match_keywords(blob, blob_dash_spaced, blob_slug)
            neg = has_negative(blob, blob_dash_spaced)
